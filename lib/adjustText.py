...
def repel_text_from_points(x, y, texts, renderer=None, ax=None,
                           prefer_move='x', expand=(1.2, 1.2), move=False):
    """
...
def adjust_text(x, y, texts, colors, ax=None, expand_text = (1.2, 1.2),
                expand_points=(1.2, 1.2), prefer_move = 'x', 
                force_text=0.5, force_points=.8, lim=100, precision=0.1,
    ...
    for j, text in enumerate(texts):
        a = ax.annotate(text.get_text(), color = colors[text.get_text()],
                        xy = (x[j], y[j]),
